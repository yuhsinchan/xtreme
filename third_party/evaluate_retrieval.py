...
def prepare_batch(
    ...
    lang="en",
    langid=None,
    use_local_max_length=True,
    pool_skip_special_token=False,
):
    ...
    if model_type == "xlm":
        langs = torch.LongTensor(
            [[langid] * max_length for _ in range(len(sentences))]
        ).to(device)
        return {
            "input_ids": input_ids,
        ...
        token_type_ids = torch.LongTensor(
            [[0] * max_length for _ in range(len(sentences))]
        ).to(device)
        return {
            "input_ids": input_ids,
            "attention_mask": attention_mask,
...
def load_model(args, lang, output_hidden_states=None):
    ...
    langid = (
        config.lang2id.get(lang, config.lang2id["en"])
    ...
    logger.info(
        "tokenizer.pad_token={}, pad_token_id={}".format(
...
def extract_embeddings(
    args, text_file, tok_file, embed_file, lang="en", pool_type="mean"
):
    ...
    all_embeds = [
        np.zeros(shape=(num_sents, args.embed_size), dtype=np.float32)
    ...
    for i in tqdm(range(num_batch), desc="Batch"):
        ...
        with torch.no_grad():
            ...
            if args.model_type == "bert" or args.model_type == "xlmr":
                ...
                first_token_outputs = last_layer_outputs[
                    :, 0
...
def main():
    ...
    parser.add_argument("--gold", default=None, help="File name of gold alignments")
    parser.add_argument(
        "--threshold", type=float, default=-1, help="Threshold (used with --output)"
    )
    parser.add_argument(
        "--embed_size", type=int, default=768, help="Dimensions of output embeddings"
    )
    parser.add_argument(
        "--pool_type", type=str, default="mean", help="pooling over work embeddings"
    )
    ...
    parser.add_argument(
        "--src_language", type=str, default="en", help="source language."
    )
    parser.add_argument(
        "--tgt_language", type=str, default="de", help="target language."
    )
    parser.add_argument("--batch_size", type=int, default=100, help="batch size.")
    parser.add_argument(
        "--tgt_text_file", type=str, default=None, help="tgt_text_file."
    )
    parser.add_argument(
        "--src_text_file", type=str, default=None, help="src_text_file."
    )
    parser.add_argument(
        "--tgt_embed_file", type=str, default=None, help="tgt_embed_file"
    )
    parser.add_argument(
        "--src_embed_file", type=str, default=None, help="src_embed_file"
    )
    ...
    parser.add_argument(
        "--predict_dir", type=str, default=None, help="prediction folder"
    )
    ...
    parser.add_argument("--unify", action="store_true", help="unify sentences")
    parser.add_argument(
        "--split", type=str, default="training", help="split of the bucc dataset"
    )
    parser.add_argument(
        "--local_rank",
        type=int,
    ...
    parser.add_argument(
        "--specific_layer", type=int, default=7, help="use specific layer"
    )
    ...
    if args.task_name == "bucc2018":
            ...
            if args.mine_bitext:
                ...
                for idx in indices:
                    ...
                    cand2score_file = os.path.join(
                        args.output_dir, "{}_{}.tsv".format(args.candidate_prefix, suf)
                    )
        ...
        for lang, candidates_single_lang in candidate_set.by_lang.items():
            ...
            for i, embedding in enumerate(embeddings):
                candidates_single_lang[i].encoding = {
                    "sentences_and_contexts": embedding
                }
        ...
        with open(
            os.path.join(root, "candidate_encodings_sentences_and_contexts.npz"), "wb"
        ) as f:
            np.save(f, np.array(list(c_id2embed_sentences_and_contexts.values())))

        print("*" * 10)
        utils_lareqa.mean_avg_prec_at_k(question_set, candidate_set, k=20)
        print("*" * 10)
        ...
        for lang, candidates_slice in candidates_by_lang.items():
            candidate_text_file = os.path.join(
                args.output_dir, f"candidates.{lang}.txt"
            )
            candidate_tok_file = os.path.join(args.output_dir, f"candidates.{lang}.tok")
            ...
            candidate_encodings = extract_encodings(
                args,
                candidate_text_file,
                candidate_tok_file,
                embed_file,
        ...
        queries = _read_jsonl(os.path.join(args.data_dir, "wikinews15_v2_xtreme.jsonl"))
        ...
        for lang, queries_slice in queries_by_lang.items():
            ...
            query_predictions_file = os.path.join(
                args.output_dir, f"queries.{lang}.predicted.txt"
            )
            ...
            with open(query_predictions_file, "w") as f:
                for i, neighbor_ids in enumerate(predictions):
                    ...
                    if i < 5:  # for debug inspection
                        logger.info(
                            "Retrieval output for '%s'" % queries_slice[i]["query"]
                        )
                        logger.info(
                            "gold=%s, predicted:" % queries_slice[i]["entity_qid"]
                        )
                        for qid in qids[:5]:
        ...
        print("*" * 10)
        logger.info(
            "Final MRR (macro-averaged over languages): %.4f" % np.mean(macro_mrr)
        )
        print("*" * 10)
        ...
        src_text_file = os.path.join(
            args.data_dir, "tatoeba.{}-eng.{}".format(src_lang2, src_lang2)
        )
        tgt_text_file = os.path.join(
            args.data_dir, "tatoeba.{}-eng.eng".format(src_lang2)
        )
        src_tok_file = os.path.join(
            args.output_dir, "tatoeba.{}-eng.tok.{}".format(src_lang2, src_lang2)
        )
        tgt_tok_file = os.path.join(
            args.output_dir, "tatoeba.{}-eng.tok.eng".format(src_lang2)
        )
        ...
        for i in [args.specific_layer]:
            ...
            with open(
                os.path.join(args.output_dir, f"test_{src_lang2}_predictions.txt"), "w"
            ) as fout:
