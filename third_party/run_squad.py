    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            inputs = {
                ...
                "token_type_ids": None
                if args.model_type in ["xlm", "xlm-roberta", "distilbert"]
                else batch[2],
...
def evaluate(args, model, tokenizer, prefix="", language="en", lang2id=None):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            inputs = {
                ...
                "attention_mask": batch[1],
                "token_type_ids": None
                if args.model_type in ["xlm", "distilbert", "xlm-roberta"]
                else batch[2],
            }
    ...
    output_prediction_file = os.path.join(
        args.output_dir, "predictions_{}_{}.json".format(language, prefix)
    )
    output_nbest_file = os.path.join(
        args.output_dir, "nbest_predictions_{}_{}.json".format(language, prefix)
    )
...
def load_and_cache_examples(
    args, tokenizer, evaluate=False, output_examples=False, language="en", lang2id=None
):
    ...
    cached_features_file = os.path.join(
        ...
        "cached_{}_{}_{}_{}".format(
            os.path.basename(args.predict_file)
            if evaluate
            else os.path.basename(args.train_file),
    ...
    if (
        ...
        if not args.data_dir and (
            ...
            examples = SquadV1Processor().get_examples_from_dataset(
                tfds_examples, evaluate=evaluate, language=language
            )
        else:
            processor = (
                SquadV2Processor()
            ...
            if evaluate:
                examples = processor.get_dev_examples(
                    args.data_dir, filename=args.predict_file, language=language
                )
            else:
                examples = processor.get_train_examples(
                    args.data_dir, filename=args.train_file, language=language
                )
...
def main():
    ...
    parser.add_argument(
        "--train_lang", type=str, default="en", help="The language of the training data"
    )
    parser.add_argument(
        "--eval_lang", type=str, default="en", help="The language of the test data"
    )
