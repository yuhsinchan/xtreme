...
def train(
    args, train_dataset, model, tokenizer, labels, pad_token_label_id, lang2id=None
):
    """Train the model."""
                ...
                if (
                    ...
                    if args.save_only_best_checkpoint:
                        ...
                        if result["f1"] > best_score:
                            logger.info(
                                "result['f1']={} > best_score={}".format(
                                    result["f1"], best_score
                                )
                            )
                            ...
                            logger.info(
                                "Saving the best model checkpoint to %s", output_dir
                            )
...
def load_and_cache_examples(
    args, tokenizer, labels, pad_token_label_id, mode, lang, lang2id=None, few_shot=-1
):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        for lg in langs:
            data_file = os.path.join(
                args.data_dir, lg, "{}.{}".format(mode, args.model_name_or_path)
            )
            logger.info(
                "Creating features from dataset file at {} in language {}".format(
                    data_file, lg
                )
            )
            if mode == "train":
                examples = read_examples_from_file(
                    data_file, lg, lang2id, args.aligned_suffix, args.augmentation
                )
            else:
        ...
        if args.local_rank in [-1, 0]:
            logger.info(
                "Saving features into cached file {}, len(features)={}".format(
    ...
    if args.model_type == "xlm" and features[0].langs is not None:
        ...
        dataset = TensorDataset(
            all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_langs
        )
    else:
        dataset = TensorDataset(
...
def main():
    ...
    parser.add_argument(
        "--predict_langs", type=str, default="en", help="prediction languages"
    )
    parser.add_argument(
        "--train_langs",
        default="en",
    ...
    parser.add_argument(
        "--aligned_suffix", type=str, default=None, help="alignment file suffix"
    )
    parser.add_argument(
        "--augmentation",
        action="store_true",
        help="reordered data with original data as augmentation",
    )
    ...
    if args.do_train:
        ...
        global_step, tr_loss = train(
            args, train_dataset, model, tokenizer, labels, pad_token_label_id, lang2id
        )
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        with open(output_test_results_file, "a") as result_writer:
            for lang in args.predict_langs.split(","):
                check_1 = not os.path.exists(
                    os.path.join(
                        args.data_dir, lang, "test.{}".format(args.model_name_or_path)
                    )
                )
                check_2 = not os.path.exists(
                    os.path.join(args.data_dir, "test-{}.tsv".format(lang))
                )
                ...
                idxfile = infile + ".idx"
                save_predictions(
                    args, predictions, output_test_predictions_file, infile, idxfile
                )
                # try:
    ...
    if args.do_predict_dev and args.local_rank in [-1, 0]:
        ...
        with open(output_test_results_file, "w") as result_writer:
            for lang in args.predict_langs.split(","):
                check_1 = not os.path.exists(
                    os.path.join(
                        args.data_dir, lang, "dev.{}".format(args.model_name_or_path)
                    )
                )
                check_2 = not os.path.exists(
                    os.path.join(args.data_dir, "dev-{}.tsv".format(lang))
                )
                if check_1 and check_2:
                    logger.info("Language {} does not exist".format(lang))
                    continue
...
def save_predictions(
    args, predictions, output_file, text_file, idx_file, output_word_prediction=False
):
    # Save predictions
