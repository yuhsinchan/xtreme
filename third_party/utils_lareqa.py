...
class QuestionSet:
    ...
    def get_encodings(self):
        return np.concatenate([np.expand_dims(q.encoding, 0) for q in self.as_list()])
...
class CandidateSet:
    ...
    def update_xling_id(self, candidate, xling_id):
        ...
        assert xling_id
        assert (
            candidate not in self.by_xling_id[xling_id]
        ), "Candidate {} already updated xling_id {}".format(candidate.uid, xling_id)
    ...
    def get_encodings(self):
        return np.concatenate([np.expand_dims(c.encoding, 0) for c in self.as_list()])
...
def set_encodings(questions, candidates, question_encoder, response_encoder):
...
def load_data(squad_per_lang):
    ...
    for lang, squad in squad_per_lang.items():
        for (
            ...
            xling_id,
            context_id,
        ) in generate_examples(squad, lang):
            question = Question(question, xling_id, lang)
            question_set.add(question)
            assert (
                answer in context_sentences
    ...
    print(
        "Totals across languages: questions={}, candidates={}".format(
        ...
        for paragraph in passage["paragraphs"]:
            ...
            for qas in paragraph["qas"]:
                ...
                if sentence:
                    ...
                    print(
                        "Warning: Skipped an example where the answer start was not "
                        "contained within any sentence. This is likely due to a "
                        "sentence breaking error."
                    )

    print(
        "Language %s: Processed %s passages, %s paragraphs, and %s questions."
...
def save_encodings(output_dir, questions, candidates):
    ...
    def write_output(output_dir, prefix, objects):
        ...
        with open(
            os.path.join(output_dir, "{}_encodings.npz".format(prefix)), "wb"
        ) as f:
            np.save(f, encodings)
        with open(os.path.join(output_dir, "{}_uids.txt".format(prefix)), "w") as f:
            for id_ in ids:
                f.write(id_ + "\n")
...
def load_encodings(output_dir, question_set, candidate_set):
    ...
    def read_input(output_dir, prefix, uid_to_obj):
        with open(
            os.path.join(output_dir, "{}_encodings.npz".format(prefix)), "rb"
        ) as f:
            encodings = np.load(f)
        with open(os.path.join(output_dir, "{}_uids.txt".format(prefix)), "r") as f:
