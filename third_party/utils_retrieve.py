...
def mine_bitext(
    ...
    if retrieval is not "bwd":
        print(
            " - perform {:d}-nn source against target, dist={}".format(
                neighborhood, dist
            )
        )
        if use_shift_embeds:
    ...
    if retrieval is not "fwd":
        print(
            " - perform {:d}-nn target against source, dist={}".format(
                neighborhood, dist
            )
        )
        if use_shift_embeds:
...
def bucc_eval(
    ...
    candidate2score = read_candidate2score(
        candidates_file, src_file, trg_file, src_id_file, trg_id_file, encoding
    )
    ...
    if gold_file is not None:
        ...
        print(
            " - best threshold={:f}: precision={:.2f}, recall={:.2f}, F1={:.2f}".format(
                threshold, 100 * precision, 100 * recall, 100 * f1
            )
        )
        return {
