...
def read_examples_from_file(
    file_path, lang, lang2id=None, aligned_suffix=None, augmentation=False
):
    ...
    if aligned_suffix:
        alignmen_file = open(
            "/".join(file_path.split("/")[:-2]) + "/train.{}".format(aligned_suffix)
        ).readlines()
    ...
    if aligned_suffix:
        ...
        reordered_words, reordered_labels = reorder_1_sent_with_labels(
            alignmen_file, all_words, all_labels
        )
